...
def save_step_visualization(
    ...
    """Save one RGB + local-traj + floorplan visualization frame for object-goal rollout."""
    if not save_images:
        return
    ...
    annotate_eval_image(
        ...
        goal_text=str(object_goal[0]),
        goal_text_panel_index=2,
        goal_text_anchor="top_right_text",
        goal_font_size=18,
    )


def eval_scene(scene_id, eval_config, worker_id):
    """Evaluate object-goal trajectories while visualizing an auxiliary point-goal rollout."""
    ...
    for traj_id in list_traj_ids(scenes_dir, scene_id)[:max_trajs_per_scene]:
        ...
        steps = 0
        image_steps = 0
        state = init_object_goal_state(env, scenes_dir, scene_id, traj_id, robot_z, device)
        ...
        while (
            ...
            with torch.no_grad():
                ...
                object_actions = infer_actions(
                    ...
                    native_depth,
                    execute_steps,
                    object_goal=state["object_goal"],
                ).squeeze(0).cpu().numpy()

            predicted_local_for_vis = object_actions[:execute_steps].copy()
            current_position = env.robots[0].get_position()[:2]
            current_yaw = env.robots[0].get_rpy()[2]
            predicted_positions = to_global_coords(
                object_actions[:execute_steps].tolist(),
...
def main(eval_config):
    """Run object-goal evaluation across the configured scene set."""
    scene_ids = list_scene_ids(
        eval_config["scenes_dir"],
        eval_config.get("scene_ids"),
        excluded_scene_ids=COMMON_EXCLUDED_SCENES,
    )
...
if __name__ == "__main__":
    ...
    scene_ids = list_scene_ids(scenes_dir, excluded_scene_ids=COMMON_EXCLUDED_SCENES)
    eval_config = {
        ...
        "traj_save_dir": str(DEFAULT_RESULTS_ROOT / "object_goal"),
    }
